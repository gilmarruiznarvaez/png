...
export function Loader() {
  ...
  useEffect(() => {
        const seen =
                typeof window !== "undefined" && sessionStorage.getItem("colablanca:seen");
        if (seen) return;
        setVisible(true);
          ...
          {visible && (
                  <motion.div
                            ...
                            <div className="flex flex-col items-center gap-8">
                                        <LogoMark className="h-16 w-16 glow-gold" />
...
/**
 * LogoMark · next/image apuntando a /logo/logo-color.svg cuando exista.
  * Fallback automático al DeerAntlers SVG inline.
   */
export function LogoMark({ className = "" }: { className?: string }) {
    ...
    return <DeerAntlers className={`${className} text-gold`} />;
}

/** Marca SVG inline de los cuernos del venado · fallback cuando no hay logo real. */
